...
const MODES = {
  ego:      { icon:'🌪', nom:'Mécanisme activé', desc:'Mon type a parlé — j\'ai réagi automatiquement.' },
  evite:    { icon:'🌫', nom:'J\'évite quelque chose', desc:'Une fuite, un retrait, un détour.' },
  presence: { icon:'✨', nom:'Présence touchée',   desc:'Un instant où je n\'étais pas dans mon mécanisme.' }
};
...
(async function init() {
  ...
  function render() {
    mount.innerHTML = `
      ...
      <h1 class="miroir-h1 fade-in-up">Un <em>instant</em></h1>
      <p class="miroir-sub fade-in-up delay-1">
        Vous avez remarqué quelque chose ? Posez-le ici en quelques secondes — pas besoin d'attendre le soir.
      </p>
      ...
      ${mode ? `
        <div class="fade-in-up">
          <p class="miroir-field__label">Quoi exactement ? <span style="text-transform:none;letter-spacing:0;color:var(--car-text-soft);font-style:italic;font-weight:400;">(une ou plusieurs)</span></p>
          ...
          <div class="miroir-field">
            <label class="miroir-field__label" for="mot">Un mot (facultatif)</label>
            <input class="miroir-input" type="text" id="mot" maxlength="60" placeholder="ex. fatigue, colère, joie…">
          </div>

          <div class="miroir-field">
            <label class="miroir-field__label" for="note">Une phrase (facultatif)</label>
            <textarea class="miroir-textarea" id="note" rows="2" placeholder="Ce qui vient — pas plus."></textarea>
          </div>

          <div class="miroir-actions">
            <a href="../aujourdhui/" class="miroir-btn miroir-btn--ghost">Plus tard</a>
            <button id="save" class="miroir-btn">Garder cet instant</button>
      ...
      ${instants.length ? `
        <section class="instants-today">
          <h2>Aujourd'hui, déjà ${instants.length} instant${instants.length>1?'s':''} déposé${instants.length>1?'s':''}</h2>
          ${instants.slice().reverse().map(it => {
            const t = it.time ? new Date(it.time).toLocaleTimeString('fr-FR', {hour:'2-digit', minute:'2-digit'}) : '—';
            return `
