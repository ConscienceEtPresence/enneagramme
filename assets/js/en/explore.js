...
const DATA_URL = "/assets/data/en/explore.json?v=2";
...
function makeNodeCard(node){
  ...
  if (node.kind === "levels" && Array.isArray(node.levels)){
    ...
    node.levels.forEach(l=>{
      ...
      b.innerHTML = `<div class="levelNum">Level ${l.n}</div><div class="levelTxt">${l.text}</div>`;
      b.addEventListener("click", ()=>{
        card.classList.add("open");
        value.style.display = "block";
        value.textContent = `Level ${l.n}: ${l.text}`;
      });
      grid.appendChild(b);
    });
...
load().catch(()=>{
  qs("#exploreSubtitle").textContent = "Unable to load assets/data/en/explore.json";
});
